"""TensorFlow/Keras decoder for V1 and hybrid V2 BASS architectures."""
...
def build_model(
    ...
    *,
    ...
    """Build a three-branch super-resolution network.

    When ``return_feature_model`` is true, also return a model exposing one
    feature tensor per searchable unit and the corresponding stable tap names.
    This is the fixed-cardinality interface intended for zero-cost proxies.
    """
    ...
    model = keras.Model(
        inputs, outputs, name=f"bass_v{spec.schema_version}_x{upscale_factor}"
    )
    if not return_feature_model:
        return model

    feature_model = keras.Model(inputs, feature_tensors, name="bass_feature_taps")
    return model, feature_model, tuple(feature_names)


# Historical public API retained.
